...
\begin{XXX}
  Introduction to the background chapter.

  Motivation.
\end{XXX}

We target two main architectures used in high-performance computing: multi-core CPUs and GPU accelerators. Their shared characteristic is that they support general-purpose programming (unlike, for example, field-programmable gate arrays (FPGAs) or application-specific integrated circuits (ASICs)). The main difference between them is that CPUs are mainly built for serial processing (possibly in multi-threaded scenarios), while GPUs are optimized for massively data-parallel workloads, running thousands of threads concurrently.



To motivate the importance of transforming data layouts and traversals in code optimization, we will show tiling: a simple optimization technique that improves the efficiency of memory accesses.
...
\xxx{In practice, there could be multiple tiling layers to optimize different hardware features (e.g., hierarchical memories, parallel or distributed execution nodes, etc.), as well as different transformations and parallelization strategies (threaded execution, GPU acceleration, distributed execution, etc.), and replacing some computations with specialized instructions, making manual rewriting of the code unnecessarily difficult and error-prone. }
...
To optimize code such as \cref{lst:matrix-multiplication} for execution on a modern multi-core CPU, we can apply the tiling optimizations as described above to improve the locality of memory accesses on their hierarchical cache systems (most significant results would be expected for the innermost loops).
...
For example, if the $B$ matrix is stored in column-major order, the innermost loop of \cref{lst:matrix-multiplication} accesses elements of $A$ and $B$ in the same contiguous manner. This allows to load the values into vector registers, which hold multiple data elements, and perform the same operation on them simultaneously using SIMD instructions, which can significantly speed up the computation.
...
\xxx{The \emph{execution model} of GPUs is based on a hierarchy of threads that are organized into blocks and grids. Blocks from the same grid are mapped to streaming multiprocessors (SMs). Threads within a block run in groups of 32 called warps, which execute instructions in lockstep (SIMT model).}
...
\emph{Memory model} of GPUs is also quite different to CPU\@. A GPU device has its own DRAM memory (global memory) that is separate from the host (CPU) memory --- thus, the computation should be organized (e.g., via tiling) in such a way that memory transfers between the host and device are balanced with the computation performance of the matrix multiplication kernel. The architecture of GPUs allows to make memory transfers and kernel executions concurrently, which can be used to hide the latency of memory transfers by overlapping them with kernel execution. Oftentimes, this can be achieved by tuning the tile sizes in a loop that sends the data between the host and device and launches the kernel on the GPU\@.
...
Each SM is equipped with a shared memory that can be used to store data that is shared among threads in the same block. In \cref{lst:tiled-matrix-multiplication}, this might be used to store the tiles of the input matrices $A$ and $B$ in beneficiary layouts for the computation of the tile of the output matrix $C$\@. The shared memory is much faster than global memory and is often used as a user-managed cache (or scratchpad memory) to reduce redundant global memory accesses. Since it is accessible by all threads in the same block, it can be also used for inter-thread communication within the block.

Since SMs have many registers (divided among the threads running on the SM\@), data structures can be also temporarily stored in registers for even faster access. However, registers are private to each thread and are inaccessible to other threads. Furthermore, all accesses to registers have to access elements at static offsets (i.e., known at compile time at the given execution point). This can be often achieved by loop unrolling; however, this also significantly increases code size.
...
There are further considerations regarding memory accesses to global memory or shared memory. One such consideration is that the shared memory is managed by 32 memory banks with limited bandwidth and so the accesses should be organized in such a way that concurrent accesses are distributed across different banks (i.e., avoiding bank conflicts). Another consideration is that global memory accesses should be coalesced, meaning that threads within a warp should ideally access contiguous memory locations and consecutive threads should access consecutive memory addresses --- then, the GPU hardware can combine these accesses into a single transaction.
